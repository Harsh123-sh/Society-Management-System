...
function NoticesPage() {
  ...
  useEffect(() => {
    ...
    fetchNotices()
      .then((response) => {
        ...
        if (mounted) setNotices(rows);
      })
      .catch(() => {
        if (mounted) {
          setNotices([]);
          setNotice("Could not load live notices.");
        }
      });
  ...
  async function submit(status) {
    ...
    try {
      const response = await createNotice(nextNotice);
      const created = response?.data || response?.notice || nextNotice;
      ...
      setNotice("Could not save notice to live data.");
    }
  }
  ...
  return (
    <article className={`notice-row ${compact ? "is-compact" : ""}`}>
      ...
      <div className="notice-row-meta">
        ...
        <span>{notice.created_at || notice.createdAt ? new Date(notice.created_at || notice.createdAt).toLocaleDateString("en-IN") : "-"}</span>
      </div>
    </article>
  );
}
